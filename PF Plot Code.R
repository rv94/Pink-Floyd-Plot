...
##GetDataset, correct to factor, remove redundant album, relevel factor and space for plotting
...
levels(PF$album_name) <- gsub(" ", "\n", levels(PF$album_name))
...
PF %>% 
  ...
  ggplot(aes(y = duration_ms/1000, x = album_name, group = album_name, colour = album_name)) +
    ...
    theme(legend.position = "none", 
          ...
          axis.text.x = element_text(angle = 0, vjust = 0.5, colour = "white"), 
